...
export function sitemapTemplate(
  ...
  <head>
  <title>XML Sitemap - Concealed Wines</title>
  ...
  <p class="expl">
    This XML Sitemap ${sitemap ? "Index file" : ""} contains ${length} ${sitemap ? "sitemaps" : "URLs"}.
  </p>
  <table id="sitemap" cellpadding="3">
  <thead>
  <tr>
  <th width="75%">${sitemap ? "Sitemap" : "URL"}</th>
...
export function rowsTemplate(urlset: { url: string; lastmod: string }[]) {
  const rows = urlset
    .map(
      (s) => `
      <tr>
